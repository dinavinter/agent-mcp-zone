...
export async function createProxyServer(request: IncomingMessage) {
    ...
    return {
        connect: async (transport: StreamableHTTPServerTransport) => {
            bindTransport({
                transportToClient: tapTransport(
                    transport,
                    tap("proxy-to-client"),
                ),
...
export default function bindTransport({
    transportToClient,
    transportToServer,
}: {
