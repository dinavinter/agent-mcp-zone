import { proxyServer, startHTTPServer, tapTransport } from "mcp-proxy";
...
const sdk = new NodeSDK({
  traceExporter: new OTLPTraceExporter({
    url: process.env.OTEL_EXPORTER_OTLP_ENDPOINT ||
      "http://aspire-dashboard:18889/v1/traces",
  }),
...
await startHTTPServer({
  ...
  createServer: async (request) => {
    const upstream = getUpstreamClientTransport(request);
...
console.log(
  `OTLP layer listening on port ${port}`,
);
...
function getUpstreamClientTransport(request: IncomingMessage):
  ...
  console.log(`OTLP layer proxying to ${target}, transport ${transportType}, `);
  ...
  if (transportType === "stdio") { 
    ...
    return new SSEClientTransport(new URL(target), {
      requestInit: {
        headers: {
          "x-otlp-layer": "true",
